...
RUN echo http://mirrors.aliyun.com/alpine/v3.10/main > /etc/apk/repositories; \
    ...
    apk add --no-cache --virtual=.build-dependencies bash libstdc++ bzip2 unzip glib libxext libxrender wget && \  # 虚拟环境
    ...
    apk add *.apk && /usr/glibc-compat/sbin/ldconfig /lib /usr/glibc/usr/lib && \  #glibc安装
    ...
    conda install -c conda-forge --no-update-deps python=3.6.5 numpy scipy pandas uwsgi pytorch-pretrained-bert Flask==1.1.1 Flask-RESTful==0.3.7 \
      WTForms==2.2.1 requests scikit-learn==0.20.3  && \  # 仅安装依赖库，不对其进行更新
    ln -s /home/user/miniconda/lib/*.58 /lib64/ && \  # uwsgi依赖库
    conda clean -tipsy && \
    rm -r "$CONDA_DIR/pkgs/" && \
